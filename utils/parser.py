...
def get_args():
    ...
    # choices=['none', 'pytorch']表示参数可以接受的值的范围
    parser.add_argument(
        '--launcher',
    ...
    parser.add_argument(
        '--deterministic',
        action='store_true',
        help='whether to set deterministic options for CUDNN backend.') # action='store_true'当这一选项存在时，为 args.deterministic 赋值为 True没有指定时则隐含地赋值为 False。      
    # bn
    parser.add_argument(
        '--sync_bn', 
        action='store_true', 
