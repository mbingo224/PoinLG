...
class PCT_encoder(nn.Module):
    ...
    def forward(self, points):
        ...
        # GDP1 /4，这里是应用 fps 获得不同分辨率的 Y，由 Y 生成 Q，这里采样率 d = 4 N = 512
        ...
        # GDP2 /2，points/2 这里是应用 fps 获得不同分辨率的 Y，由 Y 生成 Q，这里采样率 d = 8 N // 8 = 256
        ...
        # GDP3 /2 这里是应用 fps 获得不同分辨率的 Y，由 Y 生成 Q，这里采样率 d = 16 N // 16 = 128
        # x_g2: [bs, 256, 128]
...
class Model(nn.Module):
    ...
    def forward(self, x, num_query, gt=None, is_training=True):
        ...
        feat_x = feat_g
        feat_x = self.ps_adj(self.relu(self.conv(feat_x))).transpose(1,2) # 一维mlp [bs, 512, 128]->[bs, 384, 128]->[bs, 128, 384]
